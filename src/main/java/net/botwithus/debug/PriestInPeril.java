...
public class PriestInPeril {
    ...
    static Coordinate thirdfloor = new Coordinate(3408, 3483, 2);
    static Area thirddfloorArea = new Area.Circular(thirdfloor, 20);
        ...
        if (QuestVarp == 0) {
            ...
            switch (QuestVarp) {
                ...
                case 2:
                    ...
                    break;
                case 3:
                    if (player.getRegionX() > 100) {
                        ScriptConsole.println("Inside Mausoleum");
                        var entranceLadder = SceneObjectQuery.newQuery().name("Ladder").option("Climb-up").results().nearest();
                        if (entranceLadder != null) {
                            entranceLadder.interact("Climb-up");
                            return;
                        }
                        return;
                    }
                ...
                case 4:
                    if (!Backpack.contains("Golden key") && !Backpack.contains("Iron key")) {
                        ScriptConsole.println("Grabbing golden key");
                        if (templearea.contains(player)) {
                        ...
                        return;
                    } else if (Backpack.contains("Golden key") && !Backpack.contains("Iron key")) {
                        if (player.getRegionX() > 100) {
                            ScriptConsole.println("Inside Mausoleum");
                            SceneObject gate = SceneObjectQuery.newQuery().name("Gate").option("Open").results().nearest();
                            SceneObject entranceLadder = SceneObjectQuery.newQuery().name("Ladder").option("Climb-up").results().nearest();
                            if (gate == null || entranceLadder == null) {
                                return;
                            }
                            if (gate.distanceTo(player) > entranceLadder.distanceTo(player)) {
                                gate.interact("Open");
                                Execution.delay(RandomGenerator.nextInt(2400, 3000));
                                return;
                            }
                            //Get IronKey
                            ScriptConsole.println("Monument changes CBA to code manually");
...
//                            }
                            ...
                            if (insideTempleArea.contains(player)) {
                                ScriptConsole.println("Inside temple area, no iron key, exiting to go grab Iron Key");
                                SceneObject door = SceneObjectQuery.newQuery().name("Large door").results().nearest();
                                if (door != null) {
                                    door.interact("Enter");
                                    delay(RandomGenerator.nextInt(600, 800));
                            ...
                            ScriptConsole.println("Moving to mausoleum");
                            entrance = SceneObjectQuery.newQuery().name("Mausoleum").results().nearest();
                            if (!templearea.contains(player) && entrance == null) {
                                DebugScript.moveTo(templecord);
                                return;
                            }
                            entrance.interact("Enter");
                        ...
                        return;
                    } else if (Backpack.contains("Iron key") && !Backpack.contains("Bucket of murky water")) {
                        if (Backpack.contains("Bucket")) {
                            ...
                            ScriptConsole.println("Bucket not in inventory");
                            return;
                        }
                    } else if (Backpack.contains("Iron key") && Backpack.contains("Bucket of murky water")) {
                        ScriptConsole.println("Has Bucket and Key");

                        SceneObject ladder2 = SceneObjectQuery.newQuery().name("Ladder").option("Climb-up").results().nearest();
                        if (ladder2 != null) {
                            SceneObject gate = SceneObjectQuery.newQuery().name("Gate").option("Open").results().nearestTo(ladder2);
                            if (player.getY() >= gate.getCoordinate().getY()) {
                                ladder2.interact("Climb-up");
                            } else {
                                if (ladder2.distanceTo(player) > 12) {
                        ...
                        ScriptConsole.println("Outside of instance");

                        if (Backpack.contains("Iron key") && Backpack.contains("Bucket of murky water") && !thirddfloorArea.contains(player)) {
                            ScriptConsole.println("Outside Mausoleum");
                            ...
                            ScriptConsole.println("Coffin and Cell Door is not null");
                            Item ironkey = Backpack.getItem("Iron key");
                            if (Backpack.contains("Iron key") && celldoor != null) {
                ...
                case 6:
                    ...
                    if (Backpack.contains("Bucket of blessed water") && coffin2 != null) {
                        SceneObject celldoorInteract = SceneObjectQuery.newQuery().name("Cell door").option("Interact").hidden(false).results().nearest();
                        if (celldoorInteract != null) {
                            celldoorInteract.interact("Interact");
